...
describe('RemDirect', async () => {
  ...
  test('direct-list-rem', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-rem', setup.live)) return
    if (skipIfMissingIds(t, setup, ["año01","mes01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ARGENTINADATOS_TEST_REM_ENTID']
