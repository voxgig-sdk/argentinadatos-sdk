...
describe('ActaEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'acta.' + op, live)) return
    }
    ...
    let acta_ref01_data = Object.values(setup.data.existing.acta)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/acta/ActaTestData.json')
  ...
  const idmapEnvVal = process.env['ARGENTINADATOS_TEST_ACTA_ENTID']
