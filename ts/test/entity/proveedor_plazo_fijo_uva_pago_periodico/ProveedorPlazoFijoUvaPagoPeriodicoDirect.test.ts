...
describe('ProveedorPlazoFijoUvaPagoPeriodicoDirect', async () => {
  ...
  test('direct-list-proveedor_plazo_fijo_uva_pago_periodico', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-proveedor_plazo_fijo_uva_pago_periodico', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ARGENTINADATOS_TEST_PROVEEDOR_PLAZO_FIJO_UVA_PAGO_PERIODICO_ENTID']
