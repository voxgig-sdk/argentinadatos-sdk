...
describe('FondoComunInversionDirect', async () => {
  ...
  test('direct-load-fondo_comun_inversion', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-fondo_comun_inversion', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ARGENTINADATOS_TEST_FONDO_COMUN_INVERSION_ENTID']
