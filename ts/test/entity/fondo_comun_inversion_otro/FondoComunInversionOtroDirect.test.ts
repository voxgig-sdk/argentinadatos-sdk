...
describe('FondoComunInversionOtroDirect', async () => {
  ...
  test('direct-load-fondo_comun_inversion_otro', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-fondo_comun_inversion_otro', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ARGENTINADATOS_TEST_FONDO_COMUN_INVERSION_OTRO_ENTID']
