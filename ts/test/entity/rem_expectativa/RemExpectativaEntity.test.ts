...
describe('RemExpectativaEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'rem_expectativa.' + op, live)) return
    }
    ...
    let rem_expectativa_ref01_data = Object.values(setup.data.existing.rem_expectativa)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/rem_expectativa/RemExpectativaTestData.json')
  ...
  const idmapEnvVal = process.env['ARGENTINADATOS_TEST_REM_EXPECTATIVA_ENTID']
