...
describe('EntidadRendimientoEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'entidad_rendimiento.' + op, live)) return
    }
    ...
    let entidad_rendimiento_ref01_data = Object.values(setup.data.existing.entidad_rendimiento)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/entidad_rendimiento/EntidadRendimientoTestData.json')
