...
describe('TasaPlazoFijoDirect', async () => {
  ...
  test('direct-list-tasa_plazo_fijo', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-tasa_plazo_fijo', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ARGENTINADATOS_TEST_TASA_PLAZO_FIJO_ENTID']
