...
//     (\`new ArgentinadatosSDK(...)\` / \`ArgentinadatosSDK.test(...)\` become
//     \`ArgentinadatosSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ArgentinadatosSDK'
...
// rewrite real client construction (\`new ArgentinadatosSDK(...)\` and any
// \`ArgentinadatosSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ArgentinadatosSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ArgentinadatosSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
