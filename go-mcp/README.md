...
[MCP](https://modelcontextprotocol.io) server exposing the Argentinadatos SDK as
two agent tools — `argentinadatos_list` and `argentinadatos_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/argentinadatos-mcp)
make build

# 2. Provide credentials via the environment
export ARGENTINADATOS_APIKEY=sk_live_xxx
...
```jsonc
// argentinadatos_list: first page of records
{ "entity": "acta" }
{ "entity": "acta", "query": { } }

// argentinadatos_load: one record by id
{ "entity": "acta", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/argentinadatos-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user argentinadatos \
     -- "$PWD"/dist/*/argentinadatos-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `argentinadatos_list` and `argentinadatos_load` tools now appear
   in new sessions. Ask the agent to *"list acta using argentinadatos"*
   and it calls `argentinadatos_list` with `{"entity":"acta"}`.
...
```sh
export ARGENTINADATOS_APIKEY=sk_live_xxx            # API key
export ARGENTINADATOS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `argentinadatos_list` tool
...
### Call the `argentinadatos_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `argentinadatos_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `argentinadatos_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ARGENTINADATOS_APIKEY` | API key sent with every request. |
| `ARGENTINADATOS_BASE` | Optional override of the API base URL. |
