...
func TestBonosCerEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.BonosCer(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.BonosCer(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		bonosCerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.bonos_cer")))
