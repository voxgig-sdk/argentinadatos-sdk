...
func TestCuentaRemuneradaUsdEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CuentaRemuneradaUsd(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CuentaRemuneradaUsd(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		cuentaRemuneradaUsdRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.cuenta_remunerada_usd")))
