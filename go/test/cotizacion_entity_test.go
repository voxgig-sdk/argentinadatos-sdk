...
func TestCotizacionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Cotizacion(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Cotizacion(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		cotizacionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.cotizacion")))
		...
		cotizacionRef01MatchDt0 := map[string]any{
			"id": cotizacionRef01Data["id"],
		}
		cotizacionRef01DataDt0Loaded, err := cotizacionRef01Ent.Load(cotizacionRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		cotizacionRef01DataDt0LoadResult := core.ToMapAny(entityData(cotizacionRef01DataDt0Loaded))
		if cotizacionRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if cotizacionRef01DataDt0LoadResult["id"] != cotizacionRef01Data["id"] {
