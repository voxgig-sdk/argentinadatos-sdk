...
import (
	...
	omni "github.com/voxgig-sdk/argentinadatos-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ArgentinadatosError); is {
				out := map[string]any{
					"name":    "ArgentinadatosError",
...
func omniCtx(arg any, client *sdk.ArgentinadatosSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ArgentinadatosSDK); is {
			client = live
