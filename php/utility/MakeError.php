...
class ArgentinadatosMakeError
{
    public static function call(?ArgentinadatosContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ArgentinadatosContext([], null);
        }
        $op = $ctx->op ?? new ArgentinadatosOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ArgentinadatosResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ArgentinadatosError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ArgentinadatosError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ArgentinadatosError) {
