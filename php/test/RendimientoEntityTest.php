...
class RendimientoEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArgentinadatosSDK::test(null, null);
        $ent = $testsdk->Rendimiento(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = rendimiento_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "rendimiento." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARGENTINADATOS_TEST_RENDIMIENTO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $rendimiento_ref01_data = null;
        if (count($rendimiento_ref01_data_raw) > 0) {
...
function rendimiento_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/rendimiento/RendimientoTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["rendimiento01", "rendimiento02", "rendimiento03"] as $k) {
    ...
    $entid_env_raw = getenv("ARGENTINADATOS_TEST_RENDIMIENTO_ENTID");
