...
class EstadoEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArgentinadatosSDK::test(null, null);
        $ent = $testsdk->Estado(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = estado_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "estado." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARGENTINADATOS_TEST_ESTADO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $estado_ref01_data = null;
        if (count($estado_ref01_data_raw) > 0) {
...
function estado_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/estado/EstadoTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["estado01", "estado02", "estado03"] as $k) {
    ...
    $entid_env_raw = getenv("ARGENTINADATOS_TEST_ESTADO_ENTID");
