...
class ActaEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArgentinadatosSDK::test(null, null);
        $ent = $testsdk->Acta(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Acta(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Acta(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = acta_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "acta." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARGENTINADATOS_TEST_ACTA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $acta_ref01_data = null;
        if (count($acta_ref01_data_raw) > 0) {
            $acta_ref01_data = Helpers::to_map($acta_ref01_data_raw[0][1]);
        }

        // LIST
        $acta_ref01_ent = $client->Acta(null);
        $acta_ref01_match = [];

        $acta_ref01_list_result = $acta_ref01_ent->list($acta_ref01_match, null);
        $this->assertIsArray($acta_ref01_list_result);
...
function acta_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/acta/ActaTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["acta01", "acta02", "acta03"] as $k) {
    ...
    $entid_env_raw = getenv("ARGENTINADATOS_TEST_ACTA_ENTID");
