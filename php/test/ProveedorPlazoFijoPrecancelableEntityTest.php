...
class ProveedorPlazoFijoPrecancelableEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ArgentinadatosSDK::test(null, null);
        $ent = $testsdk->ProveedorPlazoFijoPrecancelable(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->ProveedorPlazoFijoPrecancelable(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->ProveedorPlazoFijoPrecancelable(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = proveedor_plazo_fijo_precancelable_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "proveedor_plazo_fijo_precancelable." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ARGENTINADATOS_TEST_PROVEEDOR_PLAZO_FIJO_PRECANCELABLE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $proveedor_plazo_fijo_precancelable_ref01_data = null;
        if (count($proveedor_plazo_fijo_precancelable_ref01_data_raw) > 0) {
...
function proveedor_plazo_fijo_precancelable_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/proveedor_plazo_fijo_precancelable/ProveedorPlazoFijoPrecancelableTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["proveedor_plazo_fijo_precancelable01", "proveedor_plazo_fijo_precancelable02", "proveedor_plazo_fijo_precancelable03"] as $k) {
    ...
    $entid_env_raw = getenv("ARGENTINADATOS_TEST_PROVEEDOR_PLAZO_FIJO_PRECANCELABLE_ENTID");
