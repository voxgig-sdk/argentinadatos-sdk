...
// Argentinadatos SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ArgentinadatosTestFeature extends ArgentinadatosBaseFeature
{
    ...
    public function init(ArgentinadatosContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ArgentinadatosContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ArgentinadatosParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ArgentinadatosContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ArgentinadatosContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ArgentinadatosParam::call($ctx, $k);
