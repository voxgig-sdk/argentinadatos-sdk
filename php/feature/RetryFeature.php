...
// Argentinadatos SDK retry feature
...
class ArgentinadatosRetryFeature extends ArgentinadatosBaseFeature
{
    ...
    public function init(ArgentinadatosContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ArgentinadatosContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ArgentinadatosContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ArgentinadatosContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
