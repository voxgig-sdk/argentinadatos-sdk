...
// Argentinadatos SDK RiesgoPai entity
...
class RiesgoPaiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RiesgoPaiEntity($this->_client, $opts);
    }

    /**
     * @param RiesgoPai|array $args RiesgoPai data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ArgentinadatosHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RiesgoPai|array The current RiesgoPai data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RiesgoPai fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ArgentinadatosHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RiesgoPai fields).
     */
    ...
    /**
     * Load a single RiesgoPai.
     *
     * @param RiesgoPaiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RiesgoPaiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RiesgoPai|array The loaded RiesgoPai as an assoc-array at the
     *   SDK boundary; throws ArgentinadatosError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ArgentinadatosHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List RiesgoPai items matching the given filter.
     *
     * @param RiesgoPaiListMatch|array|null $reqmatch Match filter (any subset
     *   of RiesgoPai fields) as an assoc-array; RiesgoPaiListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RiesgoPai[]|array A list of RiesgoPai items as assoc-arrays at
