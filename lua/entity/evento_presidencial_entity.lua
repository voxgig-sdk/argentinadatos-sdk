-- Argentinadatos SDK EventoPresidencial entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EventoPresidencialEntity = {}
EventoPresidencialEntity.__index = EventoPresidencialEntity


function EventoPresidencialEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EventoPresidencialEntity)
  self._name = "evento_presidencial"
  self._client = client
...
function EventoPresidencialEntity:get_name()
  return self._name
end


function EventoPresidencialEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EventoPresidencialEntity.new(self._client, opts)
end
...
function EventoPresidencialEntity:mark_deleted()
  self._deleted = true
end


function EventoPresidencialEntity:deleted()
  return true == self._deleted
end


function EventoPresidencialEntity:data_set(args)
  if args ~= nil then
...
function EventoPresidencialEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EventoPresidencialEntity:match_set(args)
  if args ~= nil then
...
function EventoPresidencialEntity:match_get()
...
function EventoPresidencialEntity:stream(action, args, callopts)
...
---@param reqmatch EventoPresidencialListMatch
---@param ctrl? table
---@return EventoPresidencial[]
---@return string? err
function EventoPresidencialEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function EventoPresidencialEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EventoPresidencialEntity
