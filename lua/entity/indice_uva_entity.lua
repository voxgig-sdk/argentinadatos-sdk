...
local IndiceUvaEntity = {}
IndiceUvaEntity.__index = IndiceUvaEntity


function IndiceUvaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IndiceUvaEntity)
  self._name = "indice_uva"
  self._client = client
...
function IndiceUvaEntity:get_name()
  return self._name
end


function IndiceUvaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IndiceUvaEntity.new(self._client, opts)
end
...
function IndiceUvaEntity:mark_deleted()
  self._deleted = true
end


function IndiceUvaEntity:deleted()
  return true == self._deleted
end


function IndiceUvaEntity:data_set(args)
  if args ~= nil then
...
function IndiceUvaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IndiceUvaEntity:match_set(args)
  if args ~= nil then
...
function IndiceUvaEntity:match_get()
...
function IndiceUvaEntity:stream(action, args, callopts)
...
function IndiceUvaEntity:_run_op(ctx, post_done)
  local utility = self._utility
