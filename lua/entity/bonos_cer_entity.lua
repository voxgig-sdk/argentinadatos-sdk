...
local BonosCerEntity = {}
BonosCerEntity.__index = BonosCerEntity


function BonosCerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BonosCerEntity)
  self._name = "bonos_cer"
  self._client = client
...
function BonosCerEntity:get_name()
  return self._name
end


function BonosCerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BonosCerEntity.new(self._client, opts)
end
...
function BonosCerEntity:mark_deleted()
  self._deleted = true
end


function BonosCerEntity:deleted()
  return true == self._deleted
end


function BonosCerEntity:data_set(args)
  if args ~= nil then
...
function BonosCerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BonosCerEntity:match_set(args)
  if args ~= nil then
...
function BonosCerEntity:match_get()
...
function BonosCerEntity:stream(action, args, callopts)
...
function BonosCerEntity:_run_op(ctx, post_done)
  local utility = self._utility
