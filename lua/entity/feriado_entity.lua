...
local FeriadoEntity = {}
FeriadoEntity.__index = FeriadoEntity


function FeriadoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FeriadoEntity)
  self._name = "feriado"
  self._client = client
...
function FeriadoEntity:get_name()
  return self._name
end


function FeriadoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FeriadoEntity.new(self._client, opts)
end
...
function FeriadoEntity:mark_deleted()
  self._deleted = true
end


function FeriadoEntity:deleted()
  return true == self._deleted
end


function FeriadoEntity:data_set(args)
  if args ~= nil then
...
function FeriadoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FeriadoEntity:match_set(args)
  if args ~= nil then
...
function FeriadoEntity:match_get()
...
function FeriadoEntity:stream(action, args, callopts)
...
---@param reqmatch FeriadoLoadMatch
---@param ctrl? table
---@return Feriado
---@return string? err
function FeriadoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FeriadoEntity:_run_op(ctx, post_done)
  local utility = self._utility
