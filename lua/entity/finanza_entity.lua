...
local FinanzaEntity = {}
FinanzaEntity.__index = FinanzaEntity


function FinanzaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FinanzaEntity)
  self._name = "finanza"
  self._client = client
...
function FinanzaEntity:get_name()
  return self._name
end


function FinanzaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FinanzaEntity.new(self._client, opts)
end
...
function FinanzaEntity:mark_deleted()
  self._deleted = true
end


function FinanzaEntity:deleted()
  return true == self._deleted
end


function FinanzaEntity:data_set(args)
  if args ~= nil then
...
function FinanzaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FinanzaEntity:match_set(args)
  if args ~= nil then
...
function FinanzaEntity:match_get()
...
function FinanzaEntity:stream(action, args, callopts)
...
function FinanzaEntity:_run_op(ctx, post_done)
  local utility = self._utility
