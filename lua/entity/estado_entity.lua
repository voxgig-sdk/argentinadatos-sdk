...
local EstadoEntity = {}
EstadoEntity.__index = EstadoEntity


function EstadoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EstadoEntity)
  self._name = "estado"
  self._client = client
...
function EstadoEntity:get_name()
  return self._name
end


function EstadoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EstadoEntity.new(self._client, opts)
end
...
function EstadoEntity:mark_deleted()
  self._deleted = true
end


function EstadoEntity:deleted()
  return true == self._deleted
end


function EstadoEntity:data_set(args)
  if args ~= nil then
...
function EstadoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EstadoEntity:match_set(args)
  if args ~= nil then
...
function EstadoEntity:match_get()
...
function EstadoEntity:stream(action, args, callopts)
...
function EstadoEntity:_run_op(ctx, post_done)
  local utility = self._utility
