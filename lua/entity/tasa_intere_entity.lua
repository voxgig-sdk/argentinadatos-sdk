...
local TasaIntereEntity = {}
TasaIntereEntity.__index = TasaIntereEntity


function TasaIntereEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TasaIntereEntity)
  self._name = "tasa_intere"
  self._client = client
...
function TasaIntereEntity:get_name()
  return self._name
end


function TasaIntereEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TasaIntereEntity.new(self._client, opts)
end
...
function TasaIntereEntity:mark_deleted()
  self._deleted = true
end


function TasaIntereEntity:deleted()
  return true == self._deleted
end


function TasaIntereEntity:data_set(args)
  if args ~= nil then
...
function TasaIntereEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TasaIntereEntity:match_set(args)
  if args ~= nil then
...
function TasaIntereEntity:match_get()
...
function TasaIntereEntity:stream(action, args, callopts)
...
function TasaIntereEntity:_run_op(ctx, post_done)
  local utility = self._utility
