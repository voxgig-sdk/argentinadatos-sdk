...
local ActaEntity = {}
ActaEntity.__index = ActaEntity


function ActaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActaEntity)
  self._name = "acta"
  self._client = client
...
function ActaEntity:get_name()
  return self._name
end


function ActaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActaEntity.new(self._client, opts)
end
...
function ActaEntity:mark_deleted()
  self._deleted = true
end


function ActaEntity:deleted()
  return true == self._deleted
end


function ActaEntity:data_set(args)
  if args ~= nil then
...
function ActaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActaEntity:match_set(args)
  if args ~= nil then
...
function ActaEntity:stream(action, args, callopts)
...
function ActaEntity:_run_op(ctx, post_done)
  local utility = self._utility
