...
local CotizacionEntity = {}
CotizacionEntity.__index = CotizacionEntity


function CotizacionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CotizacionEntity)
  self._name = "cotizacion"
  self._client = client
...
function CotizacionEntity:get_name()
  return self._name
end


function CotizacionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CotizacionEntity.new(self._client, opts)
end
...
function CotizacionEntity:mark_deleted()
  self._deleted = true
end


function CotizacionEntity:deleted()
  return true == self._deleted
end


function CotizacionEntity:data_set(args)
  if args ~= nil then
...
function CotizacionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CotizacionEntity:match_set(args)
  if args ~= nil then
...
function CotizacionEntity:match_get()
...
function CotizacionEntity:stream(action, args, callopts)
...
---@param reqmatch CotizacionLoadMatch
---@param ctrl? table
---@return Cotizacion
---@return string? err
function CotizacionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CotizacionListMatch
---@param ctrl? table
---@return Cotizacion[]
---@return string? err
function CotizacionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CotizacionEntity:_run_op(ctx, post_done)
  local utility = self._utility
