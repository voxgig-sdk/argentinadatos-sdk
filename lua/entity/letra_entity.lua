...
local LetraEntity = {}
LetraEntity.__index = LetraEntity


function LetraEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LetraEntity)
  self._name = "letra"
  self._client = client
...
function LetraEntity:get_name()
  return self._name
end


function LetraEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LetraEntity.new(self._client, opts)
end
...
function LetraEntity:mark_deleted()
  self._deleted = true
end


function LetraEntity:deleted()
  return true == self._deleted
end


function LetraEntity:data_set(args)
  if args ~= nil then
...
function LetraEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LetraEntity:match_set(args)
  if args ~= nil then
...
function LetraEntity:match_get()
...
function LetraEntity:stream(action, args, callopts)
...
function LetraEntity:_run_op(ctx, post_done)
  local utility = self._utility
