...
local RiesgoPaiEntity = {}
RiesgoPaiEntity.__index = RiesgoPaiEntity


function RiesgoPaiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RiesgoPaiEntity)
  self._name = "riesgo_pai"
  self._client = client
...
function RiesgoPaiEntity:get_name()
  return self._name
end


function RiesgoPaiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RiesgoPaiEntity.new(self._client, opts)
end
...
function RiesgoPaiEntity:mark_deleted()
  self._deleted = true
end


function RiesgoPaiEntity:deleted()
  return true == self._deleted
end


function RiesgoPaiEntity:data_set(args)
  if args ~= nil then
...
function RiesgoPaiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RiesgoPaiEntity:match_set(args)
  if args ~= nil then
...
function RiesgoPaiEntity:match_get()
...
function RiesgoPaiEntity:stream(action, args, callopts)
...
function RiesgoPaiEntity:_run_op(ctx, post_done)
  local utility = self._utility
