...
local SenadorEntity = {}
SenadorEntity.__index = SenadorEntity


function SenadorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SenadorEntity)
  self._name = "senador"
  self._client = client
...
function SenadorEntity:get_name()
  return self._name
end


function SenadorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SenadorEntity.new(self._client, opts)
end
...
function SenadorEntity:mark_deleted()
  self._deleted = true
end


function SenadorEntity:deleted()
  return true == self._deleted
end


function SenadorEntity:data_set(args)
  if args ~= nil then
...
function SenadorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SenadorEntity:match_set(args)
  if args ~= nil then
...
function SenadorEntity:match_get()
...
function SenadorEntity:stream(action, args, callopts)
...
---@param reqmatch SenadorListMatch
---@param ctrl? table
---@return Senador[]
---@return string? err
function SenadorEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SenadorEntity:_run_op(ctx, post_done)
  local utility = self._utility
