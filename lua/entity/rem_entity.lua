...
local RemEntity = {}
RemEntity.__index = RemEntity


function RemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RemEntity)
  self._name = "rem"
  self._client = client
...
function RemEntity:get_name()
  return self._name
end


function RemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RemEntity.new(self._client, opts)
end
...
function RemEntity:mark_deleted()
  self._deleted = true
end


function RemEntity:deleted()
  return true == self._deleted
end


function RemEntity:data_set(args)
  if args ~= nil then
...
function RemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RemEntity:match_set(args)
  if args ~= nil then
...
function RemEntity:stream(action, args, callopts)
...
---@param reqmatch RemListMatch
---@param ctrl? table
---@return Rem[]
---@return string? err
function RemEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RemEntity:_run_op(ctx, post_done)
  local utility = self._utility
