-- Argentinadatos SDK RemExpectativa entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RemExpectativaEntity = {}
RemExpectativaEntity.__index = RemExpectativaEntity


function RemExpectativaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RemExpectativaEntity)
  self._name = "rem_expectativa"
  self._client = client
...
function RemExpectativaEntity:get_name()
  return self._name
end


function RemExpectativaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RemExpectativaEntity.new(self._client, opts)
end
...
function RemExpectativaEntity:mark_deleted()
  self._deleted = true
end


function RemExpectativaEntity:deleted()
  return true == self._deleted
end


function RemExpectativaEntity:data_set(args)
  if args ~= nil then
...
function RemExpectativaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RemExpectativaEntity:match_set(args)
  if args ~= nil then
...
function RemExpectativaEntity:match_get()
...
function RemExpectativaEntity:stream(action, args, callopts)
...
---@param reqmatch RemExpectativaListMatch
---@param ctrl? table
---@return RemExpectativa[]
---@return string? err
function RemExpectativaEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RemExpectativaEntity:_run_op(ctx, post_done)
  local utility = self._utility
