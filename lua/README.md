...
The Lua SDK for the Argentinadatos API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Acta(nil):list(nil, nil)
...
```lua
local result, err = client:Acta(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Argentinadatos(nil):load(
  { id = "test01" }, nil
)
