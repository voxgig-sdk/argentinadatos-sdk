...
describe("RemDirect", function()
  it("should direct-list-rem", function()
    local setup = rem_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-rem", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"año01", "mes01"}) do
...
function rem_direct_setup(mockres)
