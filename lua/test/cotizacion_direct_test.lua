...
describe("CotizacionDirect", function()
  it("should direct-list-cotizacion", function()
    local setup = cotizacion_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-cotizacion", setup.live and "live" or "unit")
  ...
  it("should direct-load-cotizacion", function()
    local setup = cotizacion_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-cotizacion", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function cotizacion_direct_setup(mockres)
