...
describe("RiesgoPaiDirect", function()
  it("should direct-list-riesgo_pai", function()
    local setup = riesgo_pai_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-riesgo_pai", setup.live and "live" or "unit")
  ...
  it("should direct-load-riesgo_pai", function()
    local setup = riesgo_pai_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-riesgo_pai", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function riesgo_pai_direct_setup(mockres)
