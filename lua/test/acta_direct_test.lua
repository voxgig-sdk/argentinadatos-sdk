...
describe("ActaDirect", function()
  it("should direct-list-acta", function()
    local setup = acta_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-acta", setup.live and "live" or "unit")
  ...
  it("should direct-load-acta", function()
    local setup = acta_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-acta", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function acta_direct_setup(mockres)
