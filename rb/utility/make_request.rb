...
module ArgentinadatosUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ArgentinadatosResponse.new({})
    ...
    if fetch_err
      ...
      response = ArgentinadatosResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ArgentinadatosResponse.new(fetched)
    else
