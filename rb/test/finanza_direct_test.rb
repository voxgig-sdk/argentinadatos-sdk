...
class FinanzaDirectTest < Minitest::Test
  def test_direct_list_finanza
    setup = finanza_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-finanza", setup[:live] ? "live" : "unit")
    if _should_skip
...
def finanza_direct_setup(mockres)
