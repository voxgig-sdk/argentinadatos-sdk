...
class IndiceUvaDirectTest < Minitest::Test
  def test_direct_list_indice_uva
    setup = indice_uva_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-indice_uva", setup[:live] ? "live" : "unit")
    if _should_skip
...
def indice_uva_direct_setup(mockres)
