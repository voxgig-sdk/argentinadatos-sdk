...
class DiputadoDirectTest < Minitest::Test
  def test_direct_list_diputado
    setup = diputado_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-diputado", setup[:live] ? "live" : "unit")
    if _should_skip
...
def diputado_direct_setup(mockres)
