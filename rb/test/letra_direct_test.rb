...
class LetraDirectTest < Minitest::Test
  def test_direct_list_letra
    setup = letra_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-letra", setup[:live] ? "live" : "unit")
    if _should_skip
...
def letra_direct_setup(mockres)
