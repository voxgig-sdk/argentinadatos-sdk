...
class EstadoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = estado_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "estado." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ARGENTINADATOS_TEST_ESTADO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    estado_ref01_data = nil
    if estado_ref01_data_raw.length > 0
...
def estado_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "estado", "EstadoTestData.json")
  ...
  entid_env_raw = ENV["ARGENTINADATOS_TEST_ESTADO_ENTID"]
