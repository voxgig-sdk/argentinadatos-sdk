...
class TasaPlazoFijoDirectTest < Minitest::Test
  def test_direct_list_tasa_plazo_fijo
    setup = tasa_plazo_fijo_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-tasa_plazo_fijo", setup[:live] ? "live" : "unit")
    if _should_skip
...
def tasa_plazo_fijo_direct_setup(mockres)
