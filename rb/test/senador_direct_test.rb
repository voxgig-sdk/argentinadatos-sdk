...
class SenadorDirectTest < Minitest::Test
  def test_direct_list_senador
    setup = senador_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-senador", setup[:live] ? "live" : "unit")
    if _should_skip
...
def senador_direct_setup(mockres)
