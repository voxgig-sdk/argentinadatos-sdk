...
class RiesgoPaiEntity
  ...
  def make
    opts = @_entopts.dup
    RiesgoPaiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ArgentinadatosHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RiesgoPai, Hash] the current RiesgoPai data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ArgentinadatosHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RiesgoPai fields)
  def match_get
  ...
  #
  # @param reqmatch [RiesgoPaiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RiesgoPai.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RiesgoPai, Hash] the loaded RiesgoPai; raises ArgentinadatosError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ArgentinadatosHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [RiesgoPaiListMatch, Hash, nil] match filter (any subset of
  #   RiesgoPai fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<RiesgoPai>, Array] the matching RiesgoPai items; raises ArgentinadatosError on failure
