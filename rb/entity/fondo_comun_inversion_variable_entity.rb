...
class FondoComunInversionVariableEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "fondo_comun_inversion_variable"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    FondoComunInversionVariableEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.FondoComunInversionVariable.load works with no args).
