...
boru-driven command-line client **and** interactive REPL for the Argentinadatos
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/argentinadatos-cli)
make build

# 2. See usage (words, entities, env vars)
./argentinadatos-cli --help
...
./argentinadatos-cli load 1 acta            # {id:1} shorthand
./argentinadatos-cli load '{id:1}' acta       # explicit match map
./argentinadatos-cli list bonos_cer

# 5. Override the API base URL for a single call
ARGENTINADATOS_BASE=https://api.example.com ./argentinadatos-cli list acta

# 6. No arguments -> interactive REPL
./argentinadatos-cli
argentinadatos> list acta
argentinadatos> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/argentinadatos-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/argentinadatos-cli list acta
   ```
...
```sh
./argentinadatos-cli load 1 acta          # scalar shorthand for {id:1}
./argentinadatos-cli load '{id:1}' acta     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `argentinadatos>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `acta`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/argentinadatos-cli`. |
...
(`./argentinadatos-cli <expr>`) and interactively (the REPL), and expressions compose the
