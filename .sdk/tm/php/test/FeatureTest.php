...
// Argentinadatos SDK feature test
//
...
class FtCtrl extends ArgentinadatosControl
{
...
class FtClient
{
    ...
    private ArgentinadatosUtility $utility;

    public function __construct(ArgentinadatosUtility $utility, array $options)
    {
    ...
    public function get_utility(): ArgentinadatosUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ArgentinadatosContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ArgentinadatosContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ArgentinadatosContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ArgentinadatosConfig::make_feature($name);
    ...
    private static function buildUrl(ArgentinadatosSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ArgentinadatosError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ArgentinadatosResponse($fetched);
        }
    ...
    private function fail(ArgentinadatosContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ArgentinadatosContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
