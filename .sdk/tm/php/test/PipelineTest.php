...
// Argentinadatos SDK pipeline test
//
...
{
    private static function utility(): ArgentinadatosUtility
    {
        return new ArgentinadatosUtility();
    }
    ...
    private static function ctx(array $over = []): ArgentinadatosContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ArgentinadatosContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ArgentinadatosOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ArgentinadatosError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ArgentinadatosOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ArgentinadatosMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ArgentinadatosOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ArgentinadatosMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ArgentinadatosOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ArgentinadatosMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArgentinadatosSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ArgentinadatosMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ArgentinadatosResponse([]),
            'result' => new ArgentinadatosResult([]),
        ]);
        [, $err] = ArgentinadatosMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ArgentinadatosSpec([]),
            'response' => null,
            'result' => new ArgentinadatosResult([]),
        ]);
        [, $err] = ArgentinadatosMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ArgentinadatosSpec(['step' => 's']),
            'response' => new ArgentinadatosResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ArgentinadatosResult([]),
        ]);
        [, $err] = ArgentinadatosMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ArgentinadatosSpec(['step' => 's']),
            'response' => new ArgentinadatosResponse(self::resp(200, ['v' => 1])),
            'result' => new ArgentinadatosResult([]),
        ]);
        [, $err] = ArgentinadatosMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ArgentinadatosSpec(['step' => 's']),
            'response' => new ArgentinadatosResponse(self::resp(200, ['v' => 2])),
            'result' => new ArgentinadatosResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ArgentinadatosMakeResponse::call($ctx);
    ...
    {
        $preset = new ArgentinadatosResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ArgentinadatosSpec([]),
            'response' => new ArgentinadatosResponse([]),
            'result' => new ArgentinadatosResult([]),
        ...
        [$got, $err] = ArgentinadatosMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ArgentinadatosResult([])]);
        [, $err] = ArgentinadatosMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ArgentinadatosSpec([]), 'result' => null]);
        [, $err] = ArgentinadatosMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArgentinadatosSpec(['step' => 's']),
            'result' => new ArgentinadatosResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ArgentinadatosOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArgentinadatosMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArgentinadatosSpec(['step' => 's']),
            'result' => new ArgentinadatosResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ArgentinadatosOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArgentinadatosMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArgentinadatosResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ArgentinadatosSpec([]),
            'result' => new ArgentinadatosResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ArgentinadatosMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArgentinadatosMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ArgentinadatosError('boom', 'boom');
        $utility->fetcher = function (ArgentinadatosContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArgentinadatosSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ArgentinadatosMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ArgentinadatosContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ArgentinadatosMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ArgentinadatosResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ArgentinadatosMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ArgentinadatosContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArgentinadatosSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ArgentinadatosMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ArgentinadatosResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ArgentinadatosSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ArgentinadatosMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ArgentinadatosMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ArgentinadatosMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ArgentinadatosResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ArgentinadatosDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ArgentinadatosResult(['ok' => false])]);
        $this->expectException(ArgentinadatosError::class);
        ArgentinadatosDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ArgentinadatosResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ArgentinadatosMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ArgentinadatosMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ArgentinadatosBaseFeature();
        $b = new ArgentinadatosBaseFeature();
        ArgentinadatosFeatureAdd::call($ctx, $a);
        ArgentinadatosFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ArgentinadatosBaseFeature
    {
        $f = new ArgentinadatosBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ArgentinadatosFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ArgentinadatosContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ArgentinadatosSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArgentinadatosPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ArgentinadatosPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ArgentinadatosResponse(['headers' => null]),
            'result' => new ArgentinadatosResult([]),
        ]);
        ArgentinadatosResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ArgentinadatosResultBody::call($ctx);
