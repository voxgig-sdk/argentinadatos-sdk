...
The Python SDK for the Argentinadatos API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Acta(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Acta(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Argentinadatos(None).load(
    {"id": "test01"}, None
)
