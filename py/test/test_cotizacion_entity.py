...
class TestCotizacionEntity:

    def test_should_create_instance(self):
        testsdk = ArgentinadatosSDK.test(None, None)
        ent = testsdk.Cotizacion(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Cotizacion(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Cotizacion(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _cotizacion_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "cotizacion." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ARGENTINADATOS_TEST_COTIZACION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        cotizacion_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.cotizacion")))
        cotizacion_ref01_data = None
        if len(cotizacion_ref01_data_raw) > 0:
            cotizacion_ref01_data = helpers.to_map(cotizacion_ref01_data_raw[0][1])

        # LIST
        cotizacion_ref01_ent = client.Cotizacion(None)
        cotizacion_ref01_match = {}

        cotizacion_ref01_list_result = cotizacion_ref01_ent.list(cotizacion_ref01_match, None)
...
def _cotizacion_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/cotizacion/CotizacionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ARGENTINADATOS_TEST_COTIZACION_ENTID"))
