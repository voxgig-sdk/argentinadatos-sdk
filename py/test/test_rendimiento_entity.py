...
class TestRendimientoEntity:

    def test_should_create_instance(self):
        testsdk = ArgentinadatosSDK.test(None, None)
        ent = testsdk.Rendimiento(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _rendimiento_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "rendimiento." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ARGENTINADATOS_TEST_RENDIMIENTO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        rendimiento_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.rendimiento")))
        rendimiento_ref01_data = None
        if len(rendimiento_ref01_data_raw) > 0:
        ...
        rendimiento_ref01_data_dt0_loaded, err = rendimiento_ref01_ent.load(rendimiento_ref01_match_dt0, None)
        assert err is None
        assert rendimiento_ref01_data_dt0_loaded is not None



def _rendimiento_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/rendimiento/RendimientoTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ARGENTINADATOS_TEST_RENDIMIENTO_ENTID"))
