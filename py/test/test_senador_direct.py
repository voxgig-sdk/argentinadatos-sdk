...
class TestSenadorDirect:

    def test_should_direct_list_senador(self):
        setup = _senador_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-senador", "live" if setup["live"] else "unit")
        if _skip:
...
def _senador_direct_setup(mockres):
