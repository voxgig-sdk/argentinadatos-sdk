...
class TestProveedorPlazoFijoUvaPagoPeriodicoEntity:

    def test_should_create_instance(self):
        testsdk = ArgentinadatosSDK.test(None, None)
        ent = testsdk.ProveedorPlazoFijoUvaPagoPeriodico(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ProveedorPlazoFijoUvaPagoPeriodico(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ProveedorPlazoFijoUvaPagoPeriodico(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _proveedor_plazo_fijo_uva_pago_periodico_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "proveedor_plazo_fijo_uva_pago_periodico." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ARGENTINADATOS_TEST_PROVEEDOR_PLAZO_FIJO_UVA_PAGO_PERIODICO_ENTID JSON to run live")
        client = setup["client"]
        ...
        proveedor_plazo_fijo_uva_pago_periodico_ref01_data = None
        if len(proveedor_plazo_fijo_uva_pago_periodico_ref01_data_raw) > 0:
...
def _proveedor_plazo_fijo_uva_pago_periodico_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/proveedor_plazo_fijo_uva_pago_periodico/ProveedorPlazoFijoUvaPagoPeriodicoTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "ARGENTINADATOS_TEST_PROVEEDOR_PLAZO_FIJO_UVA_PAGO_PERIODICO_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ARGENTINADATOS_TEST_PROVEEDOR_PLAZO_FIJO_UVA_PAGO_PERIODICO_ENTID"))
