...
class TestFinanzaDirect:

    def test_should_direct_list_finanza(self):
        setup = _finanza_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-finanza", "live" if setup["live"] else "unit")
        if _skip:
...
def _finanza_direct_setup(mockres):
